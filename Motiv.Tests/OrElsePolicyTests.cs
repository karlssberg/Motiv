...
public class OrElsePolicyTests
{
    enum MyMetadata
    {
        Left,
    ...
    [Theory]
    [InlineAutoData(false, false, "right", "left")]
    [InlineAutoData(false, true, "right")]
    [InlineAutoData(true, false, "left")]
    [InlineAutoData(true, true, "left")]
    public void Should_yield_causes_of_result(
        bool left,
        bool right,
        params string[] expected)
    {
        // Arrange
        var leftPolicy =
            Spec.Build((bool _) => left)
                .WhenTrue(MyMetadata.Left)
                .WhenFalse(MyMetadata.NotLeft)
                .Create("left");

        var rightPolicy =
            Spec.Build((bool _) => right)
                .WhenTrue(MyMetadata.Right)
                .WhenFalse(MyMetadata.NotRight)
                .Create("right");
