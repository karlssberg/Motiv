...
import { definitionOrder } from '@motiv-rules/core';
 ...
 *
 * Mounted by {@link BuilderBody} below the rule's tree, inside the builder's tree context: each
 * definition's body is the same tree of rows the rule is, sharing the one accordion, highlight
 * and popover slot. Always present rather than only once a definition exists — an author with no
 * definitions yet still needs to be told where they will show up once `Extract to definition`
 * makes one. Rows run whole-before-parts (`definitionOrder`), so the definitions the rule uses
 * directly come first and what they are built from follows (#234).
 */
