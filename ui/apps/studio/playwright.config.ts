 ...
 * (`docker compose --profile auth up`, studio-auth on :5101 + Keycloak on :8081) via
 * MOTIV_E2E_AUTH_URL and needs no local dev-identity server at all. Starting this webServer
 * anyway would fight over host port 5100 with that stack's default `studio` service — the compose
...
const webServer = process.env.MOTIV_E2E_AUTH_URL
  ? null
  : {
      command: `dotnet run --project ../../../src/Motiv.Studio --urls ${baseURL}`,
