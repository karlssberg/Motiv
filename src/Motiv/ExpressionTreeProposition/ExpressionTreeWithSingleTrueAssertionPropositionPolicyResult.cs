...
/// <summary>
///     Represents the result of an expression-tree proposition with a single fixed true-assertion. The false
///     because-resolver is only invoked when the <see cref="Value" /> (or a property derived from it) is read for a
///     dissatisfied outcome, and all derived state is cached in fields to avoid per-evaluation lazy-wrapper and
///     closure allocations. Degenerate (null/empty/whitespace) because-strings fall back to the statement-derived
///     reason for explanation and description purposes.
/// </summary>
...
/// <param name="trueBecause">The fixed because-string used when the proposition is satisfied.</param>
/// <param name="whenFalse">Resolves the because-string for the dissatisfied outcome from the model and result.</param>
...
internal sealed class ExpressionTreeWithSingleTrueAssertionPropositionPolicyResult<TModel, TPredicateResult>(
    bool satisfied,
    TModel model,
    ...
    : PolicyResultBase<string>
{
    ...
    public override string Value
    {
        get
        {
            if (!_hasBecause)
            {
                field = Satisfied ? trueBecause : whenFalse(model, result);
                _hasBecause = true;
            }
    ...
    public override MetadataNode<string> MetadataTier =>
        field ??= new MetadataNode<string>(Value, ResultArray);
