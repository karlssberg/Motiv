...
/// <summary>
///     Represents the result of an expression-tree multi-assertion explanation proposition evaluation. The
///     assertions resolver is only invoked when the <see cref="MetadataTier" /> or <see cref="Explanation" /> is
///     read, its result is cached so it runs at most once, and all derived state is cached in fields to avoid
///     per-evaluation lazy-wrapper and closure allocations. Degenerate (null/empty/whitespace) assertions fall back
///     to the underlying result's assertions.
/// </summary>
...
internal sealed class ExpressionTreeMultiAssertionExplanationPropositionBooleanResult<TModel, TPredicateResult>(
    bool satisfied,
    TModel model,
    ...
    : BooleanResultBase<string>
{
    ...
    public override Explanation Explanation =>
        field ??= new Explanation(MetadataResults.ElseFallback(() => result.Assertions), result);
