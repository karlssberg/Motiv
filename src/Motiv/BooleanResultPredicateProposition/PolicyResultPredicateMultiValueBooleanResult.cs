...
///     <see cref="PolicyResultPredicateMultiValueProposition{TModel,TMetadata,TUnderlyingMetadata}" /> evaluation. The
///     metadata resolver is only invoked when the <see cref="MetadataTier" /> is read, and all derived state is cached
///     in fields to avoid per-evaluation lazy-wrapper and closure allocations.
/// </summary>
...
internal sealed class PolicyResultPredicateMultiValueBooleanResult<TModel, TMetadata, TUnderlyingMetadata>(
    TModel model,
    PolicyResultBase<TUnderlyingMetadata> underlyingResult,
    Func<TModel, PolicyResultBase<TUnderlyingMetadata>, IEnumerable<TMetadata>> metadataResolver,
