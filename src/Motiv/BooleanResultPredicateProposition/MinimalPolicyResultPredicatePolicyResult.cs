...
/// <summary>
///     Represents the result of a <see cref="MinimalPolicyResultPredicateProposition{TModel,TMetadata}" /> evaluation.
///     The metadata resolver is only invoked when the <see cref="Value" /> (or a property derived from it) is read, and
...
internal sealed class MinimalPolicyResultPredicatePolicyResult<TModel, TMetadata>(
    TModel model,
    PolicyResultBase<TMetadata> underlyingResult,
    Func<TModel, PolicyResultBase<TMetadata>, TMetadata> metadataResolver,
    ISpecDescription specDescription)
    : PolicyResultBase<TMetadata>
{
    private readonly PolicyResultBase<TMetadata>[] _underlyingResults = [underlyingResult];
