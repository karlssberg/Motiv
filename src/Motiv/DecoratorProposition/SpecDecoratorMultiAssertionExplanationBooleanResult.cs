...
/// <summary>
///     Represents the result of a spec-decorator multi-assertion explanation evaluation. The assertions resolver is
///     only invoked when the <see cref="MetadataTier" /> (or a property derived from it) is read, and all derived
...
internal sealed class SpecDecoratorMultiAssertionExplanationBooleanResult<TModel, TUnderlyingMetadata>(
    BooleanResultBase<TUnderlyingMetadata> booleanResult,
    TModel model,
    Func<TModel, BooleanResultBase<TUnderlyingMetadata>, IEnumerable<string>> assertionsResolver,
    ...
    : BooleanResultBase<string>
{
    ...
    private IEnumerable<string> Metadata
    {
        get
        {
            if (!_hasMetadata) { field = assertionsResolver(model, booleanResult); _hasMetadata = true; }
            return field;
        }
    } = default!;
