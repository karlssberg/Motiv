...
/// <summary>
///     Represents the result of a spec-decorator policy evaluation. The metadata resolver is only invoked when the
...
internal sealed class SpecDecoratorPolicyResult<TModel, TMetadata, TUnderlyingMetadata>(
    BooleanResultBase<TUnderlyingMetadata> booleanResult,
    TModel model,
    Func<TModel, BooleanResultBase<TUnderlyingMetadata>, TMetadata> metadataResolver,
    ...
    : PolicyResultBase<TMetadata>
{
    ...
    public override TMetadata Value
    {
        get
        {
            if (!_hasValue) { field = metadataResolver(model, booleanResult); _hasValue = true; }
            return field;
        }
    } = default!;
