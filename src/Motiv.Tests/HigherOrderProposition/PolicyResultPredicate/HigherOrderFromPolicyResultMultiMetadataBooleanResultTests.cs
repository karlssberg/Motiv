...
public class HigherOrderFromPolicyResultMultiMetadataBooleanResultTests
{
    [Theory]
    [InlineData(true, 7)]
    [InlineData(false, 9)]
    public void ExposesResultSurface(bool satisfied, int expectedMetadata)
    {
        ...
        var result = new HigherOrderFromPolicyResultMultiMetadataBooleanResult<int, int, string>(
            satisfied,
            [underlying],
            _ => [7],
            _ => [9],
