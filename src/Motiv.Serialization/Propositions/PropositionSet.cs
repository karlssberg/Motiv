...
public sealed class PropositionSet
{
    ...
    /// </summary>
    /// <remarks>
    /// Mirrors <see cref="RuleSet(SpecRegistry, IRuleStore, RuleSerializerOptions, DecisionLog)"/>, and opens a
    /// binding scope of its own over the registry. Rules that are to see these propositions must
    /// therefore be built from *this set* —
    /// <see cref="RuleSet(PropositionSet, IRuleStore, RuleSerializerOptions, DecisionLog)"/> — not from the
    ...
    public PropositionSet AddModel<TModel>(string modelTypeId) =>
        Scope.Locked(() =>
        {
            _models[modelTypeId] = PropositionModelBinding.For<TModel>(modelTypeId, _options);
            return this;
        });
