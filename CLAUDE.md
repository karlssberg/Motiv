...
- Known CodeFix edge cases to watch for: (1) `nameof()` is syntactically an `InvocationExpressionSyntax` — `InstanceMethodDetector` must not treat it as an instance method; (2) pattern-introduced variables (`obj is string s`) must not be treated as method parameters in model generation; (3) when the diagnosed expression is nested inside `!()`, the negation context must be preserved in the replacement
