...
public class BooleanResultExtensionsCountingTests
{
    ...
    private static BooleanResultBase<string>[] BooleanResults(params bool[] satisfied) =>
        satisfied.Select(s => IsPositive.Evaluate(s ? 1 : -1)).ToArray();
    ...
    private static IEnumerable<BooleanResultBase<string>> Lazy(BooleanResultBase<string>[] results) =>
    ...
    {
        var array = BooleanResults(satisfied);
