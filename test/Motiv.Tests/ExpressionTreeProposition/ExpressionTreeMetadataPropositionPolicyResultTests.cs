using System.Linq.Expressions;
using Motiv.ExpressionTreeProposition;
using Motiv.Shared;

namespace Motiv.Tests.ExpressionTreeProposition;

public class ExpressionTreeMetadataPropositionPolicyResultTests
{
    private static readonly Expression<Func<int, bool>> Expression = n => n > 0;

    private static BooleanResultBase<string> Underlying(bool satisfied) =>
        Spec.Build((int n) => n > 0)
            .WhenTrue("n > 0")
            .WhenFalse("n <= 0")
            .Create()
            .Evaluate(satisfied ? 1 : -1);

    private static ExpressionTreeMetadataPropositionPolicyResult<int, TMetadata, bool> Create<TMetadata>(
        bool satisfied,
        Func<int, BooleanResultBase<string>, TMetadata> metadataResolver) =>
        new(satisfied,
            satisfied ? 1 : -1,
            Underlying(satisfied),
            metadataResolver,
            Expression,
            new SpecDescription("is positive"));

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Satisfied_ShouldMirrorConstructorArgument(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 5);

        result.Satisfied.ShouldBe(satisfied);
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Value_ShouldReturnResolvedMetadata(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 42);

        result.Value.ShouldBe(42);
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Underlying_ShouldBeEmpty(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 5);

        result.Underlying.ShouldBeEmpty();
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void UnderlyingWithValues_ShouldBeEmpty(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 5);

        result.UnderlyingWithValues.ShouldBeEmpty();
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Causes_ShouldBeEmpty(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 5);

        result.Causes.ShouldBeEmpty();
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void CausesWithValues_ShouldBeEmpty(bool satisfied)
    {
        var result = Create(satisfied, (_, _) => 5);

        result.CausesWithValues.ShouldBeEmpty();
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Explanation_ShouldUseMetadataStringsWhenMetadataIsStringSequence(bool satisfied)
    {
        var result = Create<string[]>(satisfied, (_, _) => ["a", "b"]);

        result.Explanation.Assertions.ShouldBe(["a", "b"]);
    }

    [Theory]
    [InlineAutoData(true)]
    [InlineAutoData(false)]
    public void Explanation_ShouldCoOptUnderlyingAssertionsWhenMetadataIsNotStringSequence(bool satisfied)
    {
        var underlying = Underlying(satisfied);
        var result = new ExpressionTreeMetadataPropositionPolicyResult<int, int, bool>(
            satisfied,
            satisfied ? 1 : -1,
            underlying,
            (_, _) => 5,
            Expression,
            new SpecDescription("is positive"));

        result.Explanation.Assertions.ShouldBe(underlying.Assertions);
    }
}
