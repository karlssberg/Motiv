namespace Karlssberg.Motiv.Proposition.FirstOrderSpecBuilders.Metadata;
...
public readonly struct YieldMetadataWhenFalse<TModel, TMetadata>(
    ...
    Func<TModel, TMetadata> whenTrue)
{
    ...
    public DescriptiveMetadataSpecFactory<TModel, TMetadata> WhenFalse(TMetadata whenFalse)
    {
        whenFalse.ThrowIfNull(nameof(whenFalse));
        return new DescriptiveMetadataSpecFactory<TModel, TMetadata>(predicate, whenTrue, _ => whenFalse);
    }
    ...
    /// <returns>A specification base.</returns>
    public DescriptiveMetadataSpecFactory<TModel, TMetadata> WhenFalse(Func<TModel, TMetadata> whenFalse)
    {
        whenFalse.ThrowIfNull(nameof(whenFalse));
        return new DescriptiveMetadataSpecFactory<TModel, TMetadata>(predicate, whenTrue, whenFalse);
    }
}