...
The Fluent Builder is the entry point for creating propositions.
The builder process behaves a bit like a decision tree whereby each builder method invocation sends you down a
different path to a proposition (that is tailored to that specific use-case).
While there may be an appearance code duplication under this approach, there are actually nuanced differences between
many of these types.
Early on, it was decided to avoid over-DRYing the propositions (and their results) since this led to a perplexing
debugging experience for library maintainers.
By avoiding a knee-jerk one-size-fits-all approach, we make the code easier to understand, at the expense of some more
files and partial duplication of code.
The important thing here is to strike a balance since neither extreme is desirable.
...
flowchart LR
    ...
    FluentBuilder --> SpecBuilder(Build with Specification/Decorator function)
    FluentBuilder --> HigherOrderBuilder(Build with Higher Order Logic function)

    PredicateBuilder --> ExplanationPredicateFunction(Single Assertion)
    PredicateBuilder --> ExplanationMultiAssertionPredicateFunction(Multiple Assertions)
    PredicateBuilder --> MetadataPredicateFunction(Multiple Metadata)

    ResultBuilder --> ExplanationResultFunction(Single Assertion)
    ResultBuilder --> MultiAssertionResultFunction(Multiple Assertions)
    ResultBuilder --> MetadataResultFunction(Multiple Metadata)

    SpecBuilder --> ExplanationSpecFunction(Single Assertion)
    SpecBuilder --> MultiAssertionSpecFunction(Multiple Assertions)
    SpecBuilder --> MetadataSpecFunction(Multiple Metadata)

    HigherOrderBuilder --> ExplanationHigherOrderFunction(Single Assertion)
    HigherOrderBuilder --> MultiAssertionHigherOrderFunction(Multiple Assertions)
    HigherOrderBuilder --> MetadataHigherOrderFunction(Multiple Metadata)

    subgraph "Builders (simplified)"
        FluentBuilder
        PredicateBuilder
        ResultBuilder
        SpecBuilder
    ...
    subgraph "Propositions"
        ExplanationPredicateFunction
        ExplanationMultiAssertionPredicateFunction
        MetadataPredicateFunction
        ExplanationResultFunction
        MultiAssertionResultFunction
        MetadataResultFunction
        ExplanationSpecFunction
        MultiAssertionSpecFunction
        MetadataSpecFunction
        ExplanationHigherOrderFunction
        MultiAssertionHigherOrderFunction
        MetadataHigherOrderFunction
    end

    style FluentBuilder stroke-width:4px
```
