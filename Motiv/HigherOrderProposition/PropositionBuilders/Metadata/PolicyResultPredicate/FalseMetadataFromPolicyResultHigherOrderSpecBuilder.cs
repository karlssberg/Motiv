...
public readonly ref struct FalseMetadataFromPolicyResultHigherOrderSpecBuilder<TModel, TMetadata, TUnderlyingMetadata>(
        ...
        IEnumerable<PolicyResult<TModel, TUnderlyingMetadata>>> causeSelector)
{
    ...
    public MetadataFromPolicyResultHigherOrderPropositionFactory<TModel, TMetadata, TUnderlyingMetadata> WhenFalse(TMetadata whenFalse) =>
        new(resultResolver,
    ...
    /// <returns>An instance of <see cref="MetadataHigherOrderPropositionFactory{TModel,TMetadata,TUnderlyingMetadata}" />.</returns>
    public MetadataFromPolicyResultHigherOrderPropositionFactory<TModel, TMetadata, TUnderlyingMetadata> WhenFalse(
        Func<HigherOrderPolicyResultEvaluation<TModel, TUnderlyingMetadata>, TMetadata> whenFalse) =>
        new(resultResolver,
            higherOrderPredicate,
            whenTrue,
            results => whenFalse(results),
            causeSelector);
